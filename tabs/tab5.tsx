...
const FifthTab: React.FC<{ deviceId: string, deviceName: string }> = ({ deviceId, deviceName }) => {
  ...
  const quickCommands = [
    { label: 'start', value: 'sudo systemctl start appmais' },
    { label: 'stop', value: 'sudo systemctl stop appmais' },
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Fifth Tab</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {quickCommands.map((item, index) => (
          <View key={index} style={styles.buttonContainer}>
            <Button title={item.label} onPress={() => sendQuickCommand(item.value)} />
          </View>
        ))}
      </ScrollView>
